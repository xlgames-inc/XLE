...
{
    [Export(typeof(SchemaSourceArchiveControl))]
    [Export(typeof(IInitializable))]
    [PartCreationPolicy(CreationPolicy.NonShared)]
    class SchemaSourceArchiveControl : Control, IInitializable
    {
        void IInitializable.Initialize()
        {
            _controlHostService.RegisterControl(
                this,
                new ControlInfo(
                    "Schema Fragment Palette",
                    "Palette of schema fragments, which can be dragged into the node diagram",
                ...
                if (e.Item is Aga.Controls.Tree.TreeNodeAdv[])
                {
                    ...
                    foreach (var a in array)
                    {
                        if (a.Tag is AdaptiveSchemaSourceArchiveModel.DataBlockItem)
                        {
                            ...
                            {
                                var script = _schemaSource.FindDataBlock(archiveName);
                                if (script != null)
                                {
                                    var storage = script.CreateStorage(null, null);
                                    this.DoDragDrop(
                                        AuthoringConcept.TestNodeFactory.CreateTestNode(script, storage),

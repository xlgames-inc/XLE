//---------------------------------------------------------------------------
// <copyright file="RibbonKeyTipService.cs" company="Microsoft Corporation">
    ...
    public class KeyTipService
    {
        ...
        private string GetNextAutoGenerationKeyTip(DependencyObject targetAutoGenerationScope)
        {
            ...
            else
            {
                ...
                {
                    string langCode = culture.ThreeLetterWindowsLanguageName;
                    if (string.IsNullOrEmpty(langCode) ||
                        string.Compare(langCode, "enu", StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        alphabetsAllowed = true;
                    }
                }
                if (alphabetsAllowed)
                {
                    ...
                    if (alphaIndex < EnglishAlphabetCount)
                    {
                        return _autoGenerationKeyTipPrefix + (char)('A' + alphaIndex);
                    }
                }

                _autoGenerationKeyTipPrefix += "0";
                _nextAutoGenerationIndex = 1;
                return _autoGenerationKeyTipPrefix + "9";
            }
        }
        ...
        private const int EnglishAlphabetCount = 26;

...
namespace RenderCore { namespace GPUProfiler
{
	...
	void    Profiler::TriggerEvent(Metal::DeviceContext& context, const char name[], EventType type)
	{
		...
		newEvent._name = name;
		newEvent._type = type;
		newEvent._queryIndex = _queryPool.SetTimeStampQuery(context);
	...
	void    Profiler::Frame_Begin(Metal::DeviceContext&context, unsigned frameId)
	{
		...
		_currentQueryFrameId = _queryPool.BeginFrame(context);
	...
	void    Profiler::Frame_End(Metal::DeviceContext& context)
	{
		...
		if (_frameRecursionDepth == 0) {
			if (_currentQueryFrameId != Metal::QueryPool::FrameId_Invalid) {
				...
				_queryPool.EndFrame(context, _currentQueryFrameId);
	...
	Profiler::Profiler()
	: _queryPool(Metal::GetObjectFactory())
	{
		_currentRenderFrameId = ~unsigned(0);
	...
	Ptr CreateProfiler()
	{
		...
		} CATCH(const Metal_DX11::GPUProfiler::QueryConstructionFailure&) {
			return nullptr;
		} CATCH_END
	}

	...
	public class TreeColumn : Component
	{
        ...
        private void DrawContent(Graphics gr, Rectangle bounds, Font font)
        {
			...
			if (textSize.Width < maxTextSize.Width)
				TextRenderer.DrawText(gr, Header, font, innerBounds, Color.White/*SystemColors.ControlText*/, _baseHeaderFlags | TextFormatFlags.Left);
            else
				TextRenderer.DrawText(gr, Header, font, innerBounds, Color.White/*SystemColors.ControlText*/, _headerFlags);
        }
		...
		{
            const bool customRendering = true;
            if (customRendering)
            {
                gr.FillRectangle(Brushes.DarkGray, bounds);
                gr.DrawRectangle(SystemPens.ControlLight, bounds.X, bounds.Y, bounds.Width, bounds.Height);
            } 

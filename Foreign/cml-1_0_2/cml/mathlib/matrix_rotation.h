    ...
    Real tolerance = epsilon<Real>::placeholder())
{
    ...
    if (repeat) {
        ...
        if (s1 > tolerance) {
            ...
            angle_2 = float(sign(c1)) *     // DavidJ -- sign() always returns double (which introduces double math here)
        ...
        if (c1 > tolerance) {
            ...
            angle_0 = value_type(0);
            angle_2 = -float(sign(s1)) *    // DavidJ -- sign() always returns double (which introduces double math here)
                std::atan2(-m.basis_element(k,j),m.basis_element(j,j));
        }
    }

...
namespace Utility
{
    ...
    template <typename Type>
        static Type * PtrAdd( Type * input, ptrdiff_t offset )  { return (Type*)( size_t(input) + offset ); }

    template <typename Type> 
        static Type * AsPointer( Type * i )                     { return &(*i); }

    #if STL_ACTIVE == STL_MSVC

        template <typename Type, typename Alloc>
            static Type * AsPointer( const std::_Vector_iterator< std::_Vector_val<Type,Alloc> > & i )                               
                { return i._Ptr; }

        template <typename Type, typename Alloc>
            static const Type * AsPointer( const std::_Vector_const_iterator< std::_Vector_val<Type,Alloc> > & i )                               
                { return i._Ptr; }

        template <typename Type, typename Traits, typename Alloc>
            static Type * AsPointer(const std::_String_iterator<Type, Traits, Alloc>& i)
                { return (Type*)i._Ptr; }

        template <typename Type, typename Traits, typename Alloc>
            static const Type * AsPointer(const std::_String_const_iterator<Type, Traits, Alloc>& i)
                { return (Type*)i._Ptr; }

    #elif STL_ACTIVE == STL_GCC

//        template <typename Type> 
//            static Type * AsPointer( const typename std::vector<Type>::iterator & i )                   { return &(*i); }

        template <typename Type> 
            static const Type * AsPointer( const typename std::vector<Type>::const_iterator & i )       { return &(*i); }

        template <typename Type> 
            static Type * AsPointer( const __gnu_cxx::__normal_iterator<Type*, std::vector<Type> > & i )       { return &(*i); }

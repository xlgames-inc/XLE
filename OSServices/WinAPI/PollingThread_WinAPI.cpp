...
//////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public:
		...
		RealPollableEvent(PollableEvent::Type type) : _type(type)
		{
			if (type == PollableEvent::Type::Semaphore) {
				_platformHandle = CreateSemaphoreA(nullptr, 0, LONG_MAX, nullptr);
			} else {
				assert(_type == PollableEvent::Type::Binary);
	...
	void PollableEvent::IncreaseCounter()
	{
		auto* that = (RealPollableEvent*)this;
		assert(that->_platformHandle != INVALID_HANDLE_VALUE);
		if (that->_type == PollableEvent::Type::Binary) {
			SetEvent(that->_platformHandle);
		} else if (that->_type == PollableEvent::Type::Semaphore) {
	...
	void PollableEvent::DecreaseCounter()
	{
		// auto-decrease in Win32
		// Infact, for semaphore types decrease can only happen when a waiting thread is released
	}

	std::shared_ptr<PollableEvent> CreatePollableEvent(PollableEvent::Type type)
	{
		RealPollableEvent* result = new RealPollableEvent(type);
		// Little trick here -- PollableEvent is actually a dummy class that only provides
		// it's method signatures
		return std::shared_ptr<PollableEvent>((PollableEvent*)result);
	}

...
RWStructuredBuffer<TileCoords>	TileCoordsBuffer : register(u1);
RWTexture2D<float>				MidwayOutput : register(u2);

float CalculateNewHeight(uint3 dispatchThreadId)
{
...
{
	float newHeight = CalculateNewHeight(dispatchThreadId);
...
{
	float newHeight = MidwayOutput[dispatchThreadId.xy];
